...
function drawColouredBar(display: Display, x: number, y: number, width: number, colour: Colours) {
...
export function renderHealthBar(
  ...
  drawColouredBar(display, 0, 45, totalWidth, Colours.BarEmpty);
  ...
  for (let i = 0; i < healthText.length; i++) {
    display.drawOver(i + 1, 45, healthText[i], Colours.White, null);
  }
}
